<?php
	...
	//	}
		...
		public function insert_unique($uniq_id,$date_time){
			
				
						$result = $this->con()->query("insert into temp_order values('', '', '$uniq_id', '$date_time')");
						
						return $result;
 
		}
		public function select_book_qty($uniq_id,$Bookid){
				$result = $this->con()->query("select book_qty from temp_details where temp_id = '$uniq_id' and book_id = '$Bookid'");
				return $result;
			}
			
		public function insert_temp_details($uniq_id,$Bookid,$bqt,$Bprice){
			$result = $this->con()->query("insert into temp_details values('$uniq_id', '', '$Bookid', '$bqt', '$Bprice')");	
			return $result;
			}
