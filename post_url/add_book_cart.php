	...
	if($bqt){
		if(!$uniq_id)
		{
			...
			$_SESSION['uniq_id'] = $uniq_id;
			 ...
			 $result = $cls_additem->insert_unique($uniq_id,$date_time);

		}
	
		$maisha_q2 = $cls_additem->select_book_qty($uniq_id,$Bookid);
		...
		{	
			$inser_tem = $cls_additem->insert_temp_details($uniq_id,$Bookid,$bqt,$Bprice);
